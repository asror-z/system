...
class ZEditableSweetColumnBackup extends ZKDataColumn
{
...
HTML,
    ...
    private function value($model)
    {

        $value = ZArrayHelper::getValue($model, $this->attribute);
        $value = $this->cleanValue($value);

        switch (true) {

            case $this->column->dbType === dbTypeDateTime:
            ...
            case $this->column->dbType === dbTypeDate:
            ...
            case $this->column->dbType === dbTypeBoolean:

                $value = Az::$app->forms->wiData->valueBool($value);
        ...
        if ($this->modelColumn->pageSummary && empty($value))
            $value = 0;
        else
            if (Az::$app->forms->wiData->isEmpty($value))
                $value = Az::l('Не задано');
