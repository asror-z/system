...
class ZEditableModalColumnRavD extends ZKDataColumn
{
    ...
    public function renderDataCellContent($model, $key, $index)
    {
        ...
        $id = $model->className . '-' . $this->attribute . '-' . $index . '-' . $key;

        return ZEditRavWidget::widget([
            'readonly' => $readonly,
            'model' => $model,
            'attribute' => $this->attribute,
            'id' => $id,
            'config' => [
                'options' => ZArrayHelper::merge($this->widgetOptions, [
                    'config' => [
